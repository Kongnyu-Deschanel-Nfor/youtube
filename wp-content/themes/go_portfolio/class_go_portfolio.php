...
class GW_Go_Portfolio {
	...
	public function load_includes() {
		...
		require_once( GW_GO_PORTFOLIO_INCLUDES . 'core/class_html.php' );
		require_once( GW_GO_PORTFOLIO_INCLUDES . 'core/class_svg.php' );				
	...
	public function enqueue_styles() {
		...
		if ( !empty( $general_settings['custom-css'] ) ) wp_add_inline_style( $this->plugin_slug .'-styles', sanitize_text_field( $general_settings['custom-css'] ) );		
	...
	public function create_meta_box() {
		...
		$meta_box_fields = array( 
			...
			/*array( 
				'name' =>  __( 'Disable related videos?', 'go_portfolio_textdomain' ),
				'id' => self::$plugin_prefix . '_thumbnail_youtube_video_no_rel',
				'desc' => __( 'Whether to disable suggested videos when the video finishes.', 'go_portfolio_textdomain' ),
				'type' => 'checkbox',
				'wrapper-data-parent' => 'thumbnail-type thumbnail-video-type',
				'wrapper-data-children' => 'youtube-video'
			),*/				
			...
			/*array( 
				'name' =>  __( 'Disable related videos?', 'go_portfolio_textdomain' ),
				'id' => self::$plugin_prefix . '_lightbox_youtube_video_no_rel',
				'desc' => __( 'Whether to disable suggested videos when the video finishes.', 'go_portfolio_textdomain' ),
				'type' => 'checkbox',
				'wrapper-data-parent' => 'thumbnail-type lightbox-type lightbox-video-type',
				'wrapper-data-children' => 'youtube-video'	
					...
					if ( $portfolio['id'] == $id ) {
							...
							if ( isset( $post_types[$portfolio['post-type']] ) ) {
								...
								if ( $arg_post_type != 'attachment' || $arg_post_type == 'attachment' && isset( $portfolio['gallery-query-method'] ) && !empty( $portfolio['gallery-query-method'] ) && $portfolio['gallery-query-method'] == 'manual' ) {
									...
									/* Set correct mime types for attachments */
									if ( $arg_post_type == 'attachment' ) {
										$new_wp_query_args['post_mime_type'] = array( 'image/jpeg', 'image/jpg', 'image/gif', 'image/png', 'image/svg+xml' );
									}
                                ...
                                /* 3. Grid layout modes */
								$layout_mode = isset( $portfolio['layout-mode'] ) ? trim( $portfolio['layout-mode'] )
                                    : 'masonry';
                                if ( !in_array( $layout_mode, array( 'masonry', 'fitRows' ) ) ) $layout_mode = 'masonry';
                                $layout_mode_attr = $layout_type == 'grid' ? 'data-layout-mode="'. esc_attr(
                                        $layout_mode ) . '"' : '';
								?>						
												...
												<div id="<?php echo esc_attr( $post->ID ); ?>" class="gw-gopf-col-wrap" data-filter="<?php echo esc_attr( implode(' ', $post_term_list ) ); ?>">
													...
													if ( $lighbox_type == 'image' ) {
														...
														if ( isset( $portfolio['first-img-thumb'] ) && $portfolio['first-img-thumb'] == 'force' && isset( $matches ) && !empty( $matches ) ) {
															...
															$lb_img_file = $matches[1];
														} elseif ( !empty( $portfolio['default-img'] ) ) {
															$tn_id = (int)$portfolio['default-img'];
															$lb_img_data = wp_get_attachment_image_src( $tn_id, $lightbox_size );
															if ( $lb_img_data ) { $lb_img_file = $lb_img_data[0]; }	
														}
														...
														if ( isset( $post_meta['gw_go_portfolio_lightbox_video_type'][0] ) && $post_meta['gw_go_portfolio_lightbox_video_type'][0]== 'youtube_video' && isset( $post_meta['gw_go_portfolio_lightbox_youtube_video_id'][0] ) ) {
															//$rel = !empty( $post_meta['gw_go_portfolio_lightbox_youtube_video_no_rel'][0] ) ? '&rel=0' : '';																
															//$lighbox_link = '//www.youtube.com/watch?v=' . $post_meta['gw_go_portfolio_lightbox_youtube_video_id'][0] . $rel;
													...
													// Lightbox caption
													$lightbox_caption =  isset( $portfolio['lightbox-caption'] ) ? trim( get_the_title() ) : '';
													$lightbox_caption = apply_filters( 'go_portfolio_lightbox_caption', $lightbox_caption, $post, $portfolio );
													$post_lb_button_data_raw = '<a title="' . $lightbox_caption . '" data-id="' . $post->ID . '_' . $portfolio_key . '" href="' . $lighbox_link . '" data-content="' . $lighbox_content . '" data-mfp-src="' . $lighbox_link . '"' . ( isset( $inner_lightbox_gallery ) ? 'data-gallery="' . esc_attr( json_encode( $inner_lightbox_gallery ) ) . '"' : '' ) . ' class="' . $lighbox_class . ' gw-gopf-post-overlay gw-gopf-post-overlay-link"' . ( isset( $popup_height ) ? ' data-height="' . $popup_height . '"' : '' ) . '>';
													         $post_lb_button = '<a title="' . $lightbox_caption . '" data-id="' . $post->ID . '_' . $portfolio_key . '" href="' . $lighbox_link . '" data-content="' . $lighbox_content . '" data-mfp-src="' . $lighbox_link . '"' . ( isset( $inner_lightbox_gallery ) ? 'data-gallery="' . esc_attr( json_encode( $inner_lightbox_gallery ) ) . '"' : '' ) . ' class="' . $button_style_class . ' ' . $lighbox_class . '"' . ( isset( $popup_height ) ? ' data-height="' . $popup_height . '"' : '' ) . '>' . $button_content . '</a>';
													...
													if ( $thumbnail_type == 'image' ) {
														$tn_image_custom_class = isset( $general_settings['tn-img-custom-class'] ) ? $general_settings['tn-img-custom-class'] : '';
														// Custom class to thumbnails
														$tn_image_custom_class = apply_filters( 'go_portfolio_image_thumbnail_class', $tn_image_custom_class, $post, $portfolio );
														$tn_image_class = !empty( $tn_image_custom_class ) && is_string( $tn_image_custom_class ) ? trim( $tn_image_custom_class ) : '';
														$tn_id = null;
														$tn_img_data = null;
														$tn_img_file = null;
														$lb_img_data = null;
														...
														$first_img = !empty( $first_img ) ? $first_img[0] : '';
														
														$regex = '/((?:(?!\s|=).)*)\s*?=\s*?["\']?((?:(?<=")(?:(?<=\\\\)"|[^"])*|(?<=\')(?:(?<=\\\\)\'|[^\'])*)|(?:(?!"|\')(?:(?!\/>|>|\s).)+))/';
														preg_match_all( $regex, $first_img, $matches, PREG_SET_ORDER, 0 );
														...
														// featured image
															...
															if ( $tn_img_data ) { 
																...
																$template_data['post_media'] .= '<div class="gw-gopf-post-media-wrap-outer"><div class="gw-gopf-post-media-wrap"  style="' . ( !empty( $fixed_img_height ) ? ' height:' . $fixed_img_height . 'px' : (  ( isset( $_wrapper_img_ratio ) ? ' height:0; padding-bottom:'. $_wrapper_img_ratio * 100 . '%' : '' ) ) ) . '">';
																$template_data['post_media'] .= '<img class="'. $tn_image_class .'" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" alt="' . $alt . '" data-src="' . $tn_img_file . '" data-id="' . $tn_id . '" data-width="' . $img_w . '" data-height="' . $img_h . '" data-img-aspect-ratio="' . $real_img_ratio . '" data-view-aspect-ratio="' . $img_ratio . '" data-orientation="'. ( $img_ratio > $real_img_ratio ? 'landscape' : 'portrait' ) .'"' . ( !empty( $fixed_img_height ) ? ' data-fixh="'. $fixed_img_height .'"' :  '' ) . '>';
																$template_data['post_media'] .= '</div></div>';
																$img_ratio = null;
																$_wrapper_img_ratio = null;																														
															}
														...
														if ( isset( $portfolio['first-img-thumb'] ) && $portfolio['first-img-thumb'] == 'fallback' && empty( $img_source ) && !empty( $_img_src ) ) {
														...
														if ( !empty( $portfolio['default-img'] ) && empty( $img_source ) ) {
													...
													if ( $thumbnail_type == 'video' || $thumbnail_type == 'audio' ) {
														...
														if ( $video_type ) {
															...
															if ( $video_type == 'youtube_video' ) {	
																...
																//$rel = !empty( $post_meta['gw_go_portfolio_thumbnail_youtube_video_no_rel'][0] ) ? '&rel=0' : '';																
																//$media_embed='<iframe src="//www.youtube.com/embed/' . ( isset( $post_meta['gw_go_portfolio_thumbnail_youtube_video_id'][0] ) ?  $post_meta['gw_go_portfolio_thumbnail_youtube_video_id'][0] : '' ) . '?wmode=opaque'.$rel.'" frameborder="0" height="100%" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>';
